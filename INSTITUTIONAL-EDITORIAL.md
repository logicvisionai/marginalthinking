...
Prefer concrete verbs and objects over claims about the quality of the publication itself.
...
> We separate observed evidence from interpretation and scenarios, identify material limitations and revise assessments when new evidence changes the conclusion.
...
> Separamos evidências observadas de interpretação e cenários, indicamos limitações relevantes e revisamos avaliações quando novas evidências alteram a conclusão.

## Patterns to avoid in both languages

Do not use phrases that merely signal sophistication without adding information. In particular, avoid or rewrite:

- `reasoning boundary` / `limites do próprio raciocínio`;
- `disciplined uncertainty reduction` / `redução disciplinada da incerteza`;
- `separate signal from noise` / `separar sinal de ruído` when the sentence can say what evidence is being compared;

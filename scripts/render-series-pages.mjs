...
function collection(locale,id,s,domain=null){
  ...
  const description=domain
    ?(locale==='pt-BR'?`Pesquisas de ${domainLabel} dentro da série ${seriesLabel}.`:`${domainLabel} research within the ${seriesLabel} series.`)
    :label(s.description,locale,locale==='pt-BR'?'Pesquisa sobre energia, materiais e sistemas industriais.':'Research on energy, materials and industrial systems.');
  const items=reports.filter(r=>r.series===id&&(!domain||r.series_domain===domain));
  const domains=!domain?`<div class="research-programs">${Object.entries(s.domains||{}).map(([d,obj])=>{const n=reports.filter(r=>r.series===id&&r.series_domain===d).length;return `<article class="research-program"><span>${n}</span><h3><a href="${pagePath(locale,`${base}${d}/`)}">${esc(label(obj,locale,d))}</a></h3><p>${esc(locale==='pt-BR'?'Domínio controlado desta série editorial.':'Controlled domain within this editorial series.')}</p></article>`;}).join('')}</div>`:'';
  ...
  const chain=locale==='pt-BR'?'recurso → tecnologia → produção → infraestrutura → custo → escala → dependências → capital → capacidade/poder → consequências':'resource → technology → production → infrastructure → cost → scale → dependencies → capital → capacity/power → consequences';
  const intro=!domain?`<section class="section research-standard"><div class="container research-standard-grid"><h2>${locale==='pt-BR'?'Estrutura analítica':'Analytical chain'}</h2><p>${esc(chain)}. <a href="${pagePath(locale,'/methodology.html')}">${locale==='pt-BR'?'Método':'Method'} →</a></p></div></section>`:'';

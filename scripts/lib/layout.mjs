...
export function makeLayout(ctx){
  ...
  function nav(active='research',alternates={}){
    ...
    const labels={
      ...
      monetary:pt?'Instituições monetárias & financeiras':'Monetary & financial institutions',
      policySeries:pt?'Experimentos de política':'Policy experiments',
      hubs:pt?'Hubs permanentes':'Permanent research hubs',
      conflicts:pt?'Sistemas de Conflito':'Conflict Systems',
      data:pt?'Dados & recursos':'Data & resources',
      ...
      policyCases:pt?'Atlas de Casos de Política':'Policy Case Atlas',
    ...
    const research=group(t.nav.research,['research','research-intelligence','coverage','energy','monetary','policy-series','research-hubs','conflict-systems','data'],[
      ...
      dropLink('research-hubs','/research/hubs/',labels.hubs,pt?'Séries recorrentes consolidadas em páginas que acumulam contexto ao longo do tempo.':'Recurring research consolidated into pages that accumulate context over time.'),
      dropLink('conflict-systems','/research/conflict-systems/',labels.conflicts,pt?'Genealogias de conflito, países, percepções, doutrinas, economia política e caminhos de escalada.':'Conflict genealogies, countries, perceptions, doctrines, political economy and escalation pathways.'),
      ...
      dropLink('data','/data/',labels.data,pt?'Conjuntos estruturados, legíveis por máquina e ligados às pesquisas de origem.':'Structured, machine-readable datasets linked to source research.'),
      ...
      dropLink('monetary','/series/global-monetary-financial-institutions/',labels.monetary,pt?'Bancos centrais, instituições multilaterais, reservas e infraestrutura monetária.':'Central banks, multilateral institutions, reserves and monetary infrastructure.'),
      dropLink('policy-series','/series/policy-experiments-institutional-transitions/',labels.policySeries,pt?'Casos implementados comparados por mecanismo, execução, resultados e limites.':'Implemented cases compared by mechanism, execution, outcomes and limits.')
    ],pt?'Publicações, hubs permanentes, dados, séries e cobertura do acervo.':'Publications, permanent hubs, data, series and coverage across the research archive.');
    const intelligence=group(labels.intelligence,['actors','dependencies','opportunities','policy-cases'],[
      ...
      dropLink('policy-cases','/policy-cases/',labels.policyCases,pt?'Experiências implementadas, mecanismos, resultados e efeitos de segunda ordem.':'Implemented experiences, mechanisms, outcomes and second-order effects.')
    ...
    const mobile=`${direct('home','/index.html',t.nav.home)}${mobileSection(t.nav.research,[direct('research','/reports.html',labels.archive),direct('research-hubs','/research/hubs/',labels.hubs),direct('conflict-systems','/research/conflict-systems/',labels.conflicts),direct('research-intelligence','/research/',labels.researchIntel),direct('data','/data/',labels.data),direct('coverage','/research/coverage/',labels.coverage),direct('energy','/series/energy-materials-industrial-systems/',labels.energy),direct('monetary','/series/global-monetary-financial-institutions/',labels.monetary),direct('policy-series','/series/policy-experiments-institutional-transitions/',labels.policySeries)])}${mobileSection(labels.intelligence,[direct('actors','/actors/',labels.actors),direct('dependencies','/dependencies/',labels.dependencies),direct('opportunities','/opportunities/',labels.atlas),direct('policy-cases','/policy-cases/',labels.policyCases)])}${mobileSection(labels.explore,[direct('geography','/regions/',labels.geography),direct('search','/search.html',labels.search),direct('workspace','/workspace/',labels.workspace),direct('mcp','/mcp-docs/',labels.mcp)])}${direct('method','/methodology.html',t.nav.method)}${direct('about','/about.html',t.nav.about)}`;
  ...
  function footer(){const mcpLabel=locale==='pt-BR'?'MCP para agentes':'MCP for AI agents';return `<footer class="footer"><div class="container"><div class="footer-grid"><div class="footer-intro">${brand()}<p>${esc(t.footer.intro)}</p></div><div><div class="footer-title">${esc(t.footer.research)}</div><div class="footer-links"><a href="${localized('/reports.html')}">${esc(t.footer.archive)}</a><a href="${localized('/research/hubs/')}">${locale==='pt-BR'?'Hubs permanentes':'Research hubs'}</a><a href="${localized('/data/')}">${locale==='pt-BR'?'Dados & recursos':'Data & resources'}</a><a href="${localized('/opportunities/')}">${esc(locale==='pt-BR'?'Atlas de Oportunidades Estruturais':'Structural Opportunity Atlas')}</a><a href="${localized('/methodology.html')}">${esc(t.footer.method)}</a><a href="${localized('/workspace/')}">${locale==='pt-BR'?'Caderno de pesquisa':'Research workspace'}</a><a href="${localized('/dependencies/')}">${locale==='pt-BR'?'Rede de dependências':'Dependency network'}</a><a href="${localized('/policy-cases/')}">${locale==='pt-BR'?'Atlas de Casos de Política':'Policy Case Atlas'}</a><a href="${localized('/system-status/')}">${locale==='pt-BR'?'Registro & atualização':'Registry & freshness'}</a><a href="${localized('/feed.xml')}">RSS</a></div></div><div><div class="footer-title">${esc(t.footer.institutional)}</div><div class="footer-links"><a href="${localized('/about.html')}">${esc(t.footer.about)}</a><a href="${localized('/what-is-marginal-thinking/')}">${locale==='pt-BR'?'O que é Marginal Thinking?':'What is Marginal Thinking?'}</a><a href="${localized('/mcp-docs/')}">${esc(mcpLabel)}</a><a href="${authorUrl(author)}">${esc(author.name)}</a><a href="mailto:${esc(author.email)}">${esc(t.footer.contact)}</a></div></div></div><div class="footer-bottom"><div class="footer-meta">${esc(cfg.institution)} · ${esc(t.country||'Brazil')}</div><div class="footer-meta">© 2026 ${esc(cfg.site_name)}</div></div></div></footer><div class="product-notice" id="product-notice" role="status" aria-live="polite"></div><script type="module" src="/assets/js/research-actions.js"></script><script type="module" src="/assets/js/annotation-layer.js"></script>`;}

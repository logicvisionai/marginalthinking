...
export function makeLayout(ctx){
  ...
  function nav(active='research',alternates={}){
    ...
    const group=(label,keys,items)=>`<div class="nav-group${activeIn(keys)?' active':''}"><button class="nav-trigger" type="button" aria-haspopup="true" aria-expanded="false"><span>${esc(label)}</span><span class="nav-chevron" aria-hidden="true">⌄</span></button><div class="nav-dropdown"><div class="nav-dropdown-inner">${items.join('')}</div></div></div>`;

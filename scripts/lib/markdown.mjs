...
export function renderMarkdown(md='',options={}){
  ...
  while(i<lines.length){const l=lines[i].trim();if(!l){i++;continue;}
    ...
    if(l.includes('|')&&separator((lines[i+1]||'').trim())){const heads=split(l),rows=[];i+=2;while(i<lines.length&&lines[i].trim()&&lines[i].includes('|'))rows.push(split(lines[i++]));out.push(renderTable(heads,rows,locale,sectionTitle));continue;}

...
function mapSvg(locale){
  ...
  const pins=[...countries.values()].map(x=>{
    ...
    return '<a class="atlas-pin-link" tabindex="0" aria-label="'+esc(title)+'" data-country="'+esc(x.country.code)+'" data-name="'+esc(pick(x.country.name,locale))+'" data-counts="'+esc(JSON.stringify(Object.fromEntries(stateOrder.map(state=>[state,atlas.entries.filter(e=>e.country.code===x.country.code&&e.state===state).length]))))+'" data-states="'+esc(states)+'" href="#country-'+esc(x.country.code)+'"><g class="atlas-pin"><circle class="pin-hit" cx="'+p[0].toFixed(2)+'" cy="'+p[1].toFixed(2)+'" r="20"/><circle class="pin-halo" cx="'+p[0].toFixed(2)+'" cy="'+p[1].toFixed(2)+'" r="13"/><circle class="pin-core" cx="'+p[0].toFixed(2)+'" cy="'+p[1].toFixed(2)+'" r="9"/><text x="'+p[0].toFixed(2)+'" y="'+(p[1]+3.4).toFixed(2)+'">'+x.count+'</text><title>'+esc(title)+'</title></g></a>';
  }).join('');
...
function stateDefinitions(locale){
  const copy={
    ...
    exploitable_inefficiency:{en:'A measurable deficiency can be reduced or addressed by an actor able to earn an economic return from the solution.','pt-BR':'Uma deficiência mensurável pode ser reduzida ou contornada por um agente com condições de obter retorno econômico dessa solução.'},
    ...
    leverageable_efficiency:{en:'An existing efficient capability can support production or reduce costs for another actor.','pt-BR':'Uma capacidade eficiente já existente pode apoiar a produção ou reduzir custos de outro agente.'}
  };
  const pt=locale==='pt-BR';
  const columnLabels=pt?['Restringe valor','Permite retorno ou uso']:['Restricts value','Enables returns or use'];
...
function atlasPage(locale){
  ...
  const deck=pt?'Condições estruturais que afetam custos, produtividade, acesso a mercados e concorrência, organizadas por país.':'Structural conditions affecting costs, productivity, market access and competition, organized by country.';
  const counts=Object.fromEntries(stateOrder.map(s=>[s,atlas.entries.filter(e=>e.state===s).length]));
  const filters='<button class="active" aria-pressed="true" data-filter="all">'+(pt?'Todos':'All')+' <span>'+atlas.entries.length+'</span></button>'+stateOrder.map(s=>'<button aria-pressed="false" data-filter="'+s+'">'+esc(stateText(s,locale))+' <span>'+counts[s]+'</span></button>').join('');
  ...
  const countryLinks=countryCodes.map(code=>{const items=atlas.entries.filter(e=>e.country.code===code);return '<a href="#country-'+esc(code)+'" data-states="'+[...new Set(items.map(e=>e.state))].join(' ')+'">'+esc(pick(items[0].country.name,locale))+'</a>';}).join('');
  return '<!doctype html><html lang="'+esc(L.loc.lang)+'"><head>'+head(locale,title+' — Marginal Thinking',deck,canonical)+'</head><body class="structural-opportunity-atlas" data-locale="'+esc(locale)+'"><div class="utility-bar"><div class="container utility-row"><span>'+(pt?'Economia · Política · Sociedade':'Economics · Politics · Society')+'</span><a href="'+pagePath(locale,'/reports.html')+'">'+(pt?'Arquivo de pesquisa':'Research archive')+'</a></div></div>'+L.nav('opportunities',Object.fromEntries(Object.keys(cfg.locales).map(x=>[x,pagePath(x,'/opportunities/')])) )+'<main><section class="atlas-hero"><div class="container">'+localNav(locale,'atlas')+'<div class="atlas-hero-grid"><div><div class="eyebrow">'+(pt?'Condições econômicas':'Economic conditions')+'</div><h1>'+esc(title)+'</h1><p>'+esc(deck)+'</p></div><aside><strong>'+(pt?'Unidade de análise':'Unit of analysis')+'</strong><p>'+esc(principle)+'</p></aside></div>'+metrics(locale)+'</div></section><section class="section"><div class="container"><div class="atlas-intro"><div><div class="section-kicker">'+(pt?'Classificação':'Classification')+'</div><h2>'+(pt?'Quatro estados estruturais':'Four structural states')+'</h2></div><p>'+(pt?'Cada estado é específico à perspectiva analisada e vinculado ao mecanismo econômico documentado.':'Each state is specific to the perspective being analysed and tied to the documented economic mechanism.')+'</p></div>'+stateDefinitions(locale)+'</div></section><section class="section map-section"><div class="container"><div class="atlas-intro"><div><div class="section-kicker">'+(pt?'Cobertura atual':'Current coverage')+'</div><h2>'+(pt?'Condições monitoradas':'Monitored structural conditions')+'</h2></div><p>'+(pt?'Localização, classificação atual, última verificação e histórico de cada condição.':'Location, current classification, latest verification and history for every condition.')+'</p></div><figure class="map-panel visual-figure atlas-map-figure" data-research-visual="atlas"><figcaption>'+(pt?'Cobertura geográfica do Atlas':'Geographic coverage of the Atlas')+'</figcaption><div class="map-viewport" tabindex="0" role="region" aria-label="'+(pt?'Mapa interativo':'Interactive map')+'">'+mapSvg(locale)+'</div><div class="map-caption"><span data-map-summary>'+mapCountries+' '+(pt?(mapCountries===1?'país':'países'):(mapCountries===1?'country':'countries'))+' · '+atlas.entries.length+' '+(pt?'condições ativas':'active conditions')+'</span><span>'+(pt?'O número em cada marcador indica quantas condições estão associadas ao país. Base cartográfica: Natural Earth 1:110m · projeção Equal Earth.':'The number in each marker shows how many conditions are associated with the country. Cartographic base: Natural Earth 1:110m · Equal Earth projection.')+'</span></div><nav class="map-country-links" aria-label="'+(pt?'Países no Atlas':'Atlas countries')+'">'+countryLinks+'</nav></figure><div class="atlas-filters" role="group" aria-label="'+(pt?'Filtrar por estado':'Filter by state')+'">'+filters+'</div>'+historySummary(locale)+'</div></section><section class="section entries-section"><div class="container" id="atlas-entries">'+groups+'<div class="empty-state" hidden>'+(pt?'Nenhuma condição corresponde ao filtro selecionado.':'No conditions match the selected filter.')+'</div></div></section><section class="section atlas-method"><div class="container atlas-method-grid"><div><div class="section-kicker">'+(pt?'Rastreabilidade':'Traceability')+'</div><h2>'+(pt?'Revisões e histórico':'Revisions and history')+'</h2></div><div><p>'+(pt?'Reclassificações, mudanças de mecanismo, revisões de evidência e correções recebem um novo registro histórico. As versões anteriores permanecem disponíveis no histórico.':'Reclassifications, mechanism changes, evidence revisions and corrections receive a new history record. Previous versions remain available in the history.')+'</p><p><a href="'+pagePath(locale,'/opportunities/history/')+'">'+(pt?'Histórico do Atlas':'Atlas history')+' →</a> · <a href="'+pagePath(locale,'/methodology.html')+'">'+(pt?'Metodologia geral':'General methodology')+' →</a></p></div></div></section></main>'+L.footer()+'<script src="/assets/js/app.js"></script><script>(function(){var buttons=[].slice.call(document.querySelectorAll(".atlas-filters button"));var cards=[].slice.call(document.querySelectorAll(".opportunity-entry"));var groups=[].slice.call(document.querySelectorAll(".country-group"));var pins=[].slice.call(document.querySelectorAll(".atlas-pin-link"));var empty=document.querySelector(".empty-state");var summary=document.querySelector("[data-map-summary]");var locale=document.body.getAttribute("data-locale");buttons.forEach(function(b){b.addEventListener("click",function(){buttons.forEach(function(x){x.classList.remove("active");x.setAttribute("aria-pressed","false")});b.classList.add("active");b.setAttribute("aria-pressed","true");var f=b.getAttribute("data-filter");var visible=0;cards.forEach(function(c){var show=f==="all"||c.getAttribute("data-state")===f;c.hidden=!show;if(show)visible++});groups.forEach(function(g){g.hidden=!g.querySelector(".opportunity-entry:not([hidden])")});document.querySelectorAll(".map-country-links a").forEach(function(a){a.hidden=f!=="all"&&!(a.dataset.states||"").split(" ").includes(f)});var visibleCountries=0;pins.forEach(function(p){var states=(p.getAttribute("data-states")||"").split(" ");var show=f==="all"||states.indexOf(f)>=0;p.classList.toggle("is-filtered-out",!show);p.setAttribute("tabindex",show?"0":"-1");p.setAttribute("aria-hidden",show?"false":"true");var counts=JSON.parse(p.dataset.counts);var count=f==="all"?Object.values(counts).reduce(function(a,b){return a+b},0):counts[f]||0;p.querySelector("text").textContent=count;var pinTitle=p.dataset.name+" — "+count+" "+(locale==="pt-BR"?(count===1?"condição":"condições"):(count===1?"condition":"conditions"));p.querySelector("title").textContent=pinTitle;p.setAttribute("aria-label",pinTitle);if(show)visibleCountries++});if(summary){summary.textContent=visibleCountries+" "+(locale==="pt-BR"?(visibleCountries===1?"país":"países"):(visibleCountries===1?"country":"countries"))+" · "+visible+" "+(locale==="pt-BR"?"condições ativas":"active conditions")}empty.hidden=visible>0;});});})();</script></body></html>';
}

...
const continents=[
  [[-168,70],[-145,72],[-127,60],[-123,48],[-112,32],[-97,18],[-82,9],[-76,20],[-66,45],[-55,55],[-61,69],[-95,78],[-132,74]],
  [[-82,12],[-69,9],[-54,4],[-44,-8],[-35,-22],[-50,-55],[-68,-53],[-76,-31],[-81,-5]],
  [[-11,36],[-5,50],[12,58],[30,60],[45,56],[62,60],[80,58],[100,53],[120,48],[145,44],[160,58],[175,50],[160,30],[142,15],[120,8],[100,12],[82,20],[63,25],[45,33],[28,38],[15,45],[3,43]],
  [[-18,36],[1,37],[18,32],[31,22],[42,10],[50,-13],[36,-34],[18,-36],[6,-30],[-7,-8],[-13,10]],
  [[112,-11],[130,-12],[154,-27],[148,-42],[128,-39],[114,-31]],
  [[-53,83],[-20,81],[-18,65],[-35,59],[-52,66]]
];
...
function mapSvg(locale){
  ...
  const pins=[...countries.values()].map(x=>{
    ...
    return '<a class="atlas-pin-link" data-country="'+esc(x.country.code)+'" data-states="'+esc(states)+'" href="#country-'+esc(x.country.code)+'"><g class="atlas-pin"><circle class="pin-hit" cx="'+p[0]+'" cy="'+p[1]+'" r="19"/><circle class="pin-halo" cx="'+p[0]+'" cy="'+p[1]+'" r="13"/><circle class="pin-core" cx="'+p[0]+'" cy="'+p[1]+'" r="9"/><text x="'+p[0]+'" y="'+(p[1]+3.4)+'">'+x.count+'</text><title>'+esc(title)+'</title></g></a>';
  }).join('');
  const aria=locale==='pt-BR'?'Mapa de cobertura do Atlas com condições agregadas por país':'Atlas coverage map with conditions aggregated by country';
  return '<svg class="atlas-map" viewBox="0 0 1000 500" role="img" aria-label="'+esc(aria)+'"><g class="map-grid">'+grid+'</g><g class="map-land">'+land+'</g><g class="map-pins">'+pins+'</g></svg>';
}

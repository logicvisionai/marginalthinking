...
function page(locale){
  ...
  const description=pt?'Publicações por período, linha de pesquisa, tema e país. As edições em idiomas diferentes contam como uma única pesquisa.':'Publications by date, research program, topic and country. Language editions count as a single publication.';
  ...
  const summary=[
    ...
    [intelligence.totals.active_topics,pt?'temas abordados':'active topics'],
  ...
  return `<!doctype html><html lang="${esc(L.loc.lang)}"><head>${head}</head><body>${L.nav('research',alts)}<main><section class="page-hero"><div class="container"><div class="eyebrow dark">${pt?'ACERVO EM NÚMEROS':'RESEARCH IN NUMBERS'}</div><h1>${title}</h1><p>${description}</p></div></section><section class="section"><div class="container"><div class="intelligence-stats">${summary}</div><div class="intelligence-grid"><section class="intelligence-panel intelligence-wide"><div class="section-head"><h2>${pt?'Atividade de pesquisa':'Research activity'}</h2><span>${esc(intelligence.totals.earliest_publication)} → ${esc(intelligence.totals.latest_publication)}</span></div><div class="activity-chart">${activityBars(locale)}</div></section><section class="intelligence-panel"><div class="section-head"><h2>${pt?'Linhas de pesquisa':'Research programs'}</h2></div>${bars(locale,programEntries,{href:x=>`/research/${x.id}/`})}</section><section class="intelligence-panel"><div class="section-head"><h2>${pt?'Regiões':'Regions'}</h2></div>${bars(locale,regionEntries,{href:x=>`/regions/${x.id}/`})}</section><section class="intelligence-panel"><div class="section-head"><h2>${pt?'Países com maior cobertura':'Most-covered countries'}</h2></div>${bars(locale,countryEntries,{href:x=>`/countries/${x.slug}/`,limit:10})}</section><section class="intelligence-panel"><div class="section-head"><h2>${pt?'Temas com maior cobertura':'Most-covered topics'}</h2></div>${bars(locale,topicEntries,{href:x=>`/topics/${x.id}/`,limit:10})}</section><section class="intelligence-panel intelligence-wide"><div class="section-head"><h2>${pt?'Disponibilidade por idioma':'Language availability'}</h2></div><div class="language-grid">${langRows}</div><p class="intelligence-note">${pt?'Cada pesquisa conta uma vez, independentemente do número de traduções.':'Each publication is counted once, regardless of the number of translations.'}</p></section><section class="intelligence-panel intelligence-wide methodology-note"><h2>${pt?'Critérios de contagem':'Counting method'}</h2><p>${pt?'Os números refletem o acervo publicado até a data indicada. Uma pesquisa pode abordar vários países e temas, por isso essas categorias não devem ser somadas. A quantidade de publicações indica a extensão da cobertura, sem atribuir importância relativa aos países ou temas.':'Figures reflect the archive at the stated date. A publication can cover several countries and topics, so these categories should not be added together. Publication counts describe the extent of coverage, without assigning relative importance to countries or topics.'}</p><p class="small">${pt?'Taxonomia':'Taxonomy'} v${esc(taxonomy.version||'—')} · ${pt?'última publicação incluída':'latest publication included'} ${esc(intelligence.totals.latest_publication||'—')}</p></section></div></div></section></main>${L.footer()}<script src="/assets/js/app.js"></script></body></html>`;
}

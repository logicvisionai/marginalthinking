...
function renderComparison(){
  ...
  const rows=[
    ...
    [text('Context','Contexto'),r=>esc(r.regime)||text('Not specified','Não informado')],
    ...
    [text('Countries covered','Países abordados'),r=>list(r.countries)],
...
function exportMarkdown(selection,{personal=false,comparison=false}={}){
  ...
  for(const r of selection){
    lines.push(`## ${mdSafe(r.title)}`,'',`${r.date} · ${mdSafe(r.kindLabel)}`,'',new URL(r.url,location.origin).href,'',mdSafe(r.deck),'',`### ${text('Context','Contexto')}`,mdSafe(r.regime)||'—','',`### ${text('Key risk','Risco principal')}`,mdSafe(r.risk)||'—','',`### ${text('What to watch','O que acompanhar')}`,...r.watch.map(x=>`- ${mdSafe(x)}`),'');

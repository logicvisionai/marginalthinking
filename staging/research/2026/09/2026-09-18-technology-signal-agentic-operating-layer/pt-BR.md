...
O sinal estrutural mais importante é qualitativo: os usuários mais intensivos estão conectando agentes a ferramentas e contexto organizacional, transformando modelos de interfaces isoladas em componentes de workflows.
...
Outro trabalho da Microsoft, CORPGEN, aborda um modo diferente de falha: trabalho real com múltiplas tarefas. A Microsoft informa que agentes líderes de uso de computador caíram de **16,7% de conclusão em condição de tarefa única para 8,7% sob carga multitarefa**. Seu sistema hierárquico proposto elevou a conclusão em até 3,5× em relação a baselines, mas o sinal principal está na degradação do baseline: capacidade medida uma tarefa por vez não se transfere automaticamente para ambientes com tarefas interdependentes, memória e prioridades concorrentes. [Microsoft Research — CORPGEN](https://www.microsoft.com/en-us/research/blog/corpgen-advances-ai-agents-for-real-work/)

| Evidência | Resultado observado | O que sustenta | O que não prova |
|---|---|---|---|
| OpenAI Enterprise Signals | uso agentic é parcela grande e crescente da produção empresarial observada | agentes estão entrando em workflows reais dentro da base desse fornecedor | produtividade geral da economia |
...
| Microsoft CORPGEN | conclusão do baseline caiu de 16,7% para 8,7% em multitarefa | orquestração realista pode ser muito mais difícil do que benchmark isolado | que uma arquitetura resolva confiabilidade empresarial de forma geral |
| Agente clínico na Nature Medicine | melhor modelo on-premise alcançou cerca de 90,0% contra 90,7% do baseline em nuvem no MIRA-v2 | controle institucional local não precisa impor grande perda de capacidade nesse benchmark | prontidão clínica sem validação prospectiva |
| Draft NIST TEVV-Athlon | framework de avaliação inclui explicitamente sistemas agentic | instituições estão formalizando requisitos de teste para uso real | um padrão universal único de segurança de agentes |
...
Um estudo publicado na Nature Medicine em 15 de setembro de 2026 avaliou um agente clínico totalmente on-premise com modelos open-weight e controle institucional local. No benchmark MIRA-v2, a melhor configuração local alcançou aproximadamente **90,0% de acurácia diagnóstica**, contra **90,7%** do baseline em nuvem sob a mesma arquitetura de agente. Em um segundo benchmark, CDM, o melhor modelo on-premise chegou a **83,8%**. [Nature Medicine](https://www.nature.com/articles/s41591-026-04609-x)

O resultado estrategicamente mais útil foi o gate de confiabilidade. Usando consistência comportamental do diagnóstico, os pesquisadores mantiveram **49,4% dos casos com 98,9% de acurácia diagnóstica** em um limiar de consistência de 0,90, encaminhando os demais para revisão. Sob uma perturbação que retirou informação relevante, a acurácia total caiu de **90,6% para 70,2%**, mostrando por que desempenho nominal de benchmark não pode definir sozinho autonomia segura.
...
À medida que agentes assumem workflows mais longos e com uso de ferramentas, alguns trabalhadores migram da execução direta para especificação, revisão, tratamento de exceções e desenho de workflows. Alguns empregos podem exigir menos passos rotineiros; outros podem crescer porque a redução do custo de execução aumenta a quantidade de trabalho tentada. A gestão também pode mudar se uma pessoa conseguir supervisionar vários fluxos automatizados.

O efeito distributivo dependerá de quem controla os workflows, quem consegue redesenhá-los, como os ganhos são divididos e se trabalhadores conseguem migrar para tarefas de maior valor. Uma empresa que dê contexto e ferramentas aos agentes sem redesenhar processos pode consumir mais computação sem produzir crescimento proporcional. Uma organização que redesenhe workflows pode reduzir custo de coordenação, mas também concentrar conhecimento operacional em software e permissões.
...
Isso cria demanda por uma nova camada de infraestrutura empresarial: identidade de agentes, policy engines, harnesses de avaliação, telemetria de workflows e auditoria. O modelo pode ficar mais barato ao longo do tempo enquanto a camada de assurance ganha peso.

## O que transformaria isso em uma mudança econômica maior

A avaliação se fortaleceria se:

1. estudos independentes demonstrarem ganhos duráveis de produtividade em workflows de produção, e não apenas crescimento de uso;
...
6. workflows agentic se espalharem de early adopters para setores regulados e operacionalmente complexos;
7. custo por tarefa concluída e verificada cair mais rápido do que cresce o overhead de revisão humana.
